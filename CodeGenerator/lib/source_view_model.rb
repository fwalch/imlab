...
class SourceViewModel
  ...
  def index_assignments
    @table.indexes
      .map { |i| "#{i.name}Index[std::make_tuple(#{get_index_fields(i)})] = tid;" }
      .join("\n")
  end
  ...
  def get_index_methods(index, partial)
    index_type = "#{class_name}::#{index.name}IndexType"
    ...
    parameter_typed_variables = index.column_names
      ...
      .join(', ')
    signature = "#{return_type} #{class_name}::get(#{parameter_typed_variables})"
    if partial
      index_column_diff = @table.indexes.select { |i| i.name == index.name}[0].column_names.count - index.column_names.count
      empty_lower_columns = []
      empty_upper_columns = []
