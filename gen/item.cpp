...
void ItemStore::add(string elements[5]) {
...
void ItemStore::add_instance(int32_t i_id, int32_t i_im_id, std::string i_name, int64_t i_price, std::string i_data) {
  ...
  pkIndex[std::make_tuple(this->i_id[tid])] = tid;
  tid++;
}

void ItemStore::remove(uint64_t tid) {
...
uint64_t ItemStore::get(int32_t i_id) {
  return pkIndex[std::make_tuple(i_id)];
}
