...
void CustomerStore::add_instance(int32_t c_id, int32_t c_d_id, int32_t c_w_id, std::string c_first, std::string c_middle, std::string c_last, std::string c_street_1, std::string c_street_2, std::string c_city, std::string c_state, std::string c_zip, std::string c_phone, uint64_t c_since, std::string c_credit, int64_t c_credit_lim, int64_t c_discount, int64_t c_balance, int64_t c_ytd_paymenr, int64_t c_payment_cnt, int64_t c_delivery_cnt, std::string c_data) {
  ...
  customer_wdlIndex.insert(std::make_pair(std::make_tuple(this->c_w_id[tid], this->c_d_id[tid], this->c_last[tid], this->c_first[tid]), tid));
  c_lastIndex.insert(std::make_pair(std::make_tuple(this->c_last[tid]), tid));
  tid++;
}
...
void CustomerStore::remove(uint64_t tid) {
  ...
  auto customer_wdlIt = customer_wdlIndex.find(customer_wdlKey);
  customer_wdlIndex.erase(customer_wdlIt);
  auto c_lastKey = std::make_tuple(this->c_last[tid]);
  auto c_lastIt = c_lastIndex.find(c_lastKey);
  c_lastIndex.erase(c_lastIt);
...
std::pair<CustomerStore::customer_wdlIndexType::iterator, CustomerStore::customer_wdlIndexType::iterator> CustomerStore::getByCustomer_wdl(int32_t c_w_id, int32_t c_d_id, std::string c_last, std::string c_first) {
  return std::make_pair(
    customer_wdlIndex.lower_bound(std::make_tuple(c_w_id, c_d_id, c_last, c_first)),
    customer_wdlIndex.upper_bound(std::make_tuple(c_w_id, c_d_id, c_last, c_first)));
}

std::pair<CustomerStore::c_lastIndexType::iterator, CustomerStore::c_lastIndexType::iterator> CustomerStore::getByC_last(std::string c_last) {
  return std::make_pair(
    c_lastIndex.lower_bound(std::make_tuple(c_last)),
    c_lastIndex.upper_bound(std::make_tuple(c_last)));
}
