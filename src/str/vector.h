 ...
 */
namespace str {
  template<size_t C> class vector : protected std::vector<char> {
    private:
  ...
  template<size_t C> size_t vector<C>::size() {
    return std::vector<char>::size() / realSize;
  }

  template<size_t C> char* vector<C>::operator[](size_t pos) {
    return &std::vector<char>::operator[](pos * realSize);
  }

  template<size_t C> void vector<C>::push_back(const char* str) {
    size_t i;
  ...
  template<size_t C> void vector<C>::push_back(std::string str) {
    this->push_back(str.c_str());
  }

  template<size_t C> void vector<C>::pop_back() {
    for (size_t i = 0; i < realSize; i++) {
      std::vector<char>::pop_back();
    }
  }

  template<size_t C> void vector<C>::set(size_t index, std::string value) {
    for (size_t i = 0; i < realSize; i++) {
